<template>
    <q-drawer v-model="menuStore.isMenuOpen" :width="260" class="bg-dark text-secondary" show-if-above>
        <div v-if="menuStore.isMobileMode" class="text-center q-mt-md">
            <router-link style="font-size: 1.5rem" to="/">
                MyPopcornList
                <q-tooltip class="bg-primary">{{ $t('back_home') }}</q-tooltip>
            </router-link>
        ...
        <q-list padding>
            ...
            <MenuItem :title="$t('topanimes')" class="menu-item" icon="animation" link="/top/anime" />
...
watch(
    () => $q.screen.lt.md,
    isMobile => {
        menuStore.setMobileMode(isMobile)
    }
)
</script>
